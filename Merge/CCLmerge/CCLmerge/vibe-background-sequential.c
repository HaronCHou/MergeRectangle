...
vibeModel_Sequential_t *libvibeModel_Sequential_New()
{
  ...
  model->numberOfSamples         = 20;//样本模型个数为20
  model->matchingThreshold       = 20;//color匹配阈值为20
  model->matchingNumber          = 2;//2个
  model->updateFactor            = 1;//更新率为16
...
uint8_t *GetFgCount(const vibeModel_Sequential_t *model){
	assert(model != NULL); return(model->fgCount);
}
...
int32_t libvibeModel_Sequential_AllocInit_8u_C1R(
  ...
  //给前景计数申请内存
  model->fgCount = NULL;
  model->fgCount = (uint8_t*)malloc(width * height * sizeof(uint8_t));
...
int32_t libvibeModel_Sequential_Segmentation_8u_C1R(
  ...
  for (int index = width * height - 1; index >= 0; --index) {
	  if ((image_data[index] - historyImage[index]) * (image_data[index] - historyImage[index]) > matchingThreshold * matchingThreshold)
  ...
  for (int i = 1; i < NUMBER_OF_HISTORY_IMAGES; ++i) {
    ...
    for (int index = width * height - 1; index >= 0; --index) {
		if ((image_data[index] - pels[index])*(image_data[index] - pels[index]) <= matchingThreshold * matchingThreshold)
  ...
  for (int index = width * height - 1; index >= 0; --index) {
    if (segmentation_map[index] > 0) {
      ...
      for (int i = numberOfTests; i > 0; --i, ++indexHistoryBuffer) {
		  if ((currentValue - historyBuffer[indexHistoryBuffer]) * (currentValue - historyBuffer[indexHistoryBuffer]) <= matchingThreshold * matchingThreshold) {
          --segmentation_map[index];
  ...
  for (uint8_t *mask = segmentation_map,*p=model->fgCount, i=0; mask < segmentation_map + (width * height);++i, ++p,++mask)
  {
	  ...
	  if (*mask > 0){
		  ...
		  *p = *p + 1;//弹幕运算符，*与++是同级运算符，就看结合方向，这两个都是右到左的顺序，所以优先++再是*，必须加括号
//前景计数不应该出现在这里
		  if (*p >=  10)//连续两次标记为前景，则强制置为背景
		  {
			  *mask = COLOR_BACKGROUND;
#if 0 
			  //要更新此处的模型，不用更领域的模型
			  int index = rand() % 20;//随机选择一个模型更新
			  if (index < NUMBER_OF_HISTORY_IMAGES)//这是前两个图像帧的处理方式
				  historyImage[i + index * width * height] = image_data[i];//image_data为当前像素值
			  else
				  historyBuffer[i * 18 + index - 2] = image_data[i];//一维索引为i，18个模型，index-2
#endif
		  }
...
int32_t libvibeModel_Sequential_Update_8u_C1R(
  ...
  //memcpy(hist, model->historyImage, 2*width*height*sizeof(uint8_t));
  //memcpy(bgmodel, model->historyBuffer, 18 * width*height*sizeof(uint8_t));
  return(0);
}
  ...
  for (int index = width * height - 1; index >= 0; --index) {
    if (segmentation_map[index] > 0) {
      ...
      for (int i = numberOfTests; i > 0; --i, indexHistoryBuffer += 3) {
        ...
        /* Swaping: Putting found value in history image buffer. */
        temp_r = swappingImageBuffer[(3 * index)];
        temp_g = swappingImageBuffer[(3 * index) + 1];
        temp_b = swappingImageBuffer[(3 * index) + 2];
